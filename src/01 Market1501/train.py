...
def train_epoch(train_loader, model, criterion_reid, criterion_binary, criterion_multiclass, optimizer, device, epoch, config):
    model.train()
    running_loss = 0.0
    reid_correct = 0
    reid_total = 0
    ...
    for batch_idx, data in enumerate(tqdm(train_loader, desc=f'Training Epoch {epoch+1}', leave=False)):
        ...
        loss_binary = sum(criterion_binary(outputs[task], labels_binary[task].float()) for task in labels_binary)
        loss_multiclass = sum(criterion_multiclass(outputs[task], labels_multiclass[task]) for task in labels_multiclass)

        upper_color_loss = sum(criterion_binary(outputs['upper_colors'][i], upper_color_labels[i].float()) for i in range(len(upper_color_labels)))
        lower_color_loss = sum(criterion_binary(outputs['lower_colors'][i], lower_color_labels[i].float()) for i in range(len(lower_color_labels)))

        total_loss = loss_reid + loss_binary + loss_multiclass + upper_color_loss + lower_color_loss
        total_loss.backward()
        optimizer.step()
...
def main():
    ...
    for epoch in range(config['epochs']):
        train_metrics = train_epoch(train_loader, model, criterion_reid, criterion_binary, criterion_multiclass, optimizer, device, epoch, config)
        scheduler.step()
